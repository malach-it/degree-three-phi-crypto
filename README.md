...
`group_amount = 2^max_depth`. Each role's compact `did:key` amount token adds a
context-derived BLS point scaled by `2^hop`, binding the commitment, group,
role, and depth while leaving the base group token separate for algebraic
custody chaining. Chain validation requires every disclosure group to start at
hop 0 and advance one hop at a time without changing its commitment or maximum
depth.
